...
export class CommonSocketEventsHandler {
  ...
  public static handleNewSocket(
    ...
    socket.on(COMMON_SOCKET_EVENT_TYPES.SOCKET_JOIN_ROOM, (data: any) => {
      CommonSocketEventsHandler.SOCKET_JOIN_ROOM(io, socket, data);
    });

    socket.on(COMMON_SOCKET_EVENT_TYPES.SOCKET_LEAVE_ROOM, (data: any) => {
      CommonSocketEventsHandler.SOCKET_LEAVE_ROOM(io, socket, data);
    });

    socket.on(COMMON_SOCKET_EVENT_TYPES.SOCKET_TRACK, (data: any) => {
  ...
  private static SOCKET_JOIN_ROOM(
    ...
    if (!validEventData) {
      socket.to(socket.id).emit(`ERROR`, {
        error: `${COMMON_SOCKET_EVENT_TYPES.SOCKET_JOIN_ROOM}-error`,
  ...
  private static SOCKET_LEAVE_ROOM(
    ...
    if (!validEventData) {
      socket.to(socket.id).emit(`ERROR`, {
        error: `${COMMON_SOCKET_EVENT_TYPES.SOCKET_LEAVE_ROOM}-error`,
        message: `room is required.`
      });
      return;
    }
  ...
  private static async SOCKET_TRACK(
    ...
    try {
      ...
      console.log(e);
      io.to(socket.id).emit(`${COMMON_SOCKET_EVENT_TYPES.SOCKET_TRACK}-error`, { message: `jwt is invalid.`, e });
      return;
    }

    if (!you) {
      io.to(socket.id).emit(`${COMMON_SOCKET_EVENT_TYPES.SOCKET_TRACK}-error`, { message: `jwt is invalid.` });
      return;
    }
