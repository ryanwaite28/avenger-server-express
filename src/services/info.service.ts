...
import { NewsDataCache } from '../models/avenger.model';
...
export class InfoService {
  ...
  static async get_business_news(): ServiceMethodAsyncResults {
    ...
    if (result) {
      // check age
      const olderThanOneDay = InfoService.moreThanOneDayAgo(result.get('created_at'));
      if (olderThanOneDay) {
        // old data, delete from db and get new
        await result.destroy();
      } else {
        // not old enough, load to cache and return
        InfoService.newsApiResults = JSON.parse(result.dataValues.json_data);
