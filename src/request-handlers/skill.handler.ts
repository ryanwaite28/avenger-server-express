...
export class SkillRequestHandler {
  ...
  @CatchRequestHandlerError()
  static async get_user_skills_requests_submitted_all(request: Request, response: Response): ExpressResponse {
    const user_id: number = parseInt(request.params.user_id, 10);
    const serviceMethodResults: ServiceMethodResults = await SkillService.get_user_skills_requests_submitted_all(user_id);
    return response.status(serviceMethodResults.status).json(serviceMethodResults.info);
  }

  @CatchRequestHandlerError()
  static async get_user_skills_requests_submitted(request: Request, response: Response): ExpressResponse {
    const user_id: number = parseInt(request.params.user_id, 10);
    const user_skill_id: number = parseInt(request.params.user_skill_id, 10);
    const serviceMethodResults: ServiceMethodResults = await SkillService.get_user_skills_requests_submitted(user_id, user_skill_id);
    return response.status(serviceMethodResults.status).json(serviceMethodResults.info);
  }
  ...
  @CatchRequestHandlerError()
  static async check_user_skill_submission_pending(request: Request, response: Response): ExpressResponse {
    const dto: UserSkillAddDto = response.locals[`dto`];
    const serviceMethodResults: ServiceMethodResults = await SkillService.check_user_skill_submission_pending(dto);
    return response.status(serviceMethodResults.status).json(serviceMethodResults.info);
  }
  @CatchRequestHandlerError()
  static async submit_user_skill(request: Request, response: Response): ExpressResponse {
    const dto: UserSkillAddDto = response.locals[`dto`];
    const serviceMethodResults: ServiceMethodResults = await SkillService.submit_user_skill(dto);
    return response.status(serviceMethodResults.status).json(serviceMethodResults.info);
  }
  @CatchRequestHandlerError()
  static async cancel_user_skill_submission(request: Request, response: Response): ExpressResponse {
    const skill_submission_request_id: number = parseInt(request.params.skill_submission_request_id, 10);
    const serviceMethodResults: ServiceMethodResults = await SkillService.cancel_user_skill_submission(skill_submission_request_id);
    return response.status(serviceMethodResults.status).json(serviceMethodResults.info);
  }
  @CatchRequestHandlerError()
  static async accept_user_skill_submission(request: Request, response: Response): ExpressResponse {
    const skill_submission_request_id: number = parseInt(request.params.skill_submission_request_id, 10);
    const serviceMethodResults: ServiceMethodResults = await SkillService.accept_user_skill_submission(skill_submission_request_id);
    return response.status(serviceMethodResults.status).json(serviceMethodResults.info);
  }
  @CatchRequestHandlerError()
  static async decline_user_skill_submission(request: Request, response: Response): ExpressResponse {
    const skill_submission_request_id: number = parseInt(request.params.skill_submission_request_id, 10);
    const serviceMethodResults: ServiceMethodResults = await SkillService.decline_user_skill_submission(skill_submission_request_id);
    return response.status(serviceMethodResults.status).json(serviceMethodResults.info);
  }
